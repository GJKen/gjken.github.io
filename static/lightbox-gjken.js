	...
	class Lightbox {
      ...
      `;
		...
		bindEvents() {
			...
			this.imageWrapper.addEventListener('click', this.close.bind(this));
		}
		...
		open() {
			...
			// 获取图片元素
			let ImgOpen = document.querySelector('.lb-lightbox-image');

			// 防止图片被新标签打开
			ImgOpen.addEventListener("dragstart", function(event) {
				event.preventDefault(); // 禁止拖动默认行为
			});
			ImgOpen.addEventListener("click", function(event) {
				event.stopPropagation(); // 阻止点击事件冒泡
			});
			// 鼠标按下事件，初始化拖动
			ImgOpen.addEventListener("mousedown", function(event) {
				console.log("图片被按下了");
				...
				// 获取当前图片的位置
				const rect = ImgOpen.getBoundingClientRect();

				// 获取图片当前的缩放比例（scale）
				const transform = window.getComputedStyle(ImgOpen).transform;
				const scale = transform === 'none' ? 1 : parseFloat(transform.split(',')[0].replace(
					'matrix(', ''));
				...
				if (scale === 1) {
					offsetX = startX - rect.left; // 鼠标相对于图片左上角的偏移
					offsetY = startY - rect.top; // 鼠标相对于图片左上角的偏移
				} else {
					// 如果scale不为1，调整偏移量
					offsetX = (startX - rect.left) / scale;
					offsetY = (startY - rect.top) / scale;
				}

				console.log(`起始坐标: (${startX}, ${startY})`);
				console.log(`图片初始位置: left: ${rect.left}, top: ${rect.top}`);
				console.log(`鼠标相对图片的偏移: offsetX: ${offsetX}, offsetY: ${offsetY}`);

				// 设置图片为绝对定位
				ImgOpen.style.position = 'absolute';
				...
				function onMouseMove(moveEvent) {
					...
					// 可以添加边界检测，确保图片不会被拖出可视区域
					const maxLeft = window.innerWidth - ImgOpen.offsetWidth;
					const maxTop = window.innerHeight - ImgOpen.offsetHeight;

					const finalLeft = Math.max(0, Math.min(newLeft, maxLeft));
					const finalTop = Math.max(0, Math.min(newTop, maxTop));

					// 更新图片位置
					ImgOpen.style.left = finalLeft + 'px';
					ImgOpen.style.top = finalTop + 'px';

					console.log(`当前鼠标位置: (${moveEvent.clientX}, ${moveEvent.clientY})`);
					console.log(`图片新位置: left: ${finalLeft}, top: ${finalTop}`);
				}
